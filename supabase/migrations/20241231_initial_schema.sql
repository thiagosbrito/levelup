-- LevelUp Family MVP - Database Schema Migration
-- This creates the complete database schema for family management, tasks, events, messaging, and gamification
...
CREATE TABLE tasks (
    ...
    recurrence_pattern TEXT, -- JSON string for cron-like patterns
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL,
...
CREATE TABLE achievement_definitions (
    ...
    criteria TEXT NOT NULL, -- JSON string defining achievement criteria
...
CREATE TABLE points_transactions (
    ...
    reference_id UUID, -- Reference to related entity (task, achievement, etc.)
    description TEXT NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() NOT NULL
);
...
-- Trigger to award points when tasks are completed
CREATE OR REPLACE FUNCTION handle_task_completion()
RETURNS TRIGGER AS $$
BEGIN
    -- Only award points when task status changes to 'completed'
...
-- User Profiles: Users can read their own profile and profiles of family members
...
-- Family Members: Users can see family members of families they belong to
...
-- Tasks: Users can see tasks from families they belong to
...
-- Family Events: Users can see events from families they belong to
...
-- Family Messages: Users can see messages from families they belong to
...
-- Insert some default achievement definitions
